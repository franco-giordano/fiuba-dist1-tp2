...
class ShardedJoinerController:
    def __init__(self, rabbit_ip, shard_exchange_name, output_exchange_name, assigned_shard_key, next_reducers_amount):
        ...
        self.matches_joiner = MatchesJoiner(self.channel, output_exchange_name, next_reducers_amount)

    def run(self):
        logging.info(f'Q3 JOINER {self.assigned_shard_key}: Waiting for messages. To exit press CTRL+C')
        self.channel.start_consuming()
        self.connection.close()

    def _callback(self, ch, method, properties, body):
        if BatchEncoderDecoder.is_encoded_sentinel(body):
            logging.info(f"Q3 JOINER {self.assigned_shard_key}: Received sentinel! Flushing and shutting down...")
            self.matches_joiner.received_sentinel()
            # TODO: shutdown my node
            return

        batch = BatchEncoderDecoder.decode_bytes(body)
        logging.info(f'Q3 JOINER {self.assigned_shard_key}: Received batch {body[:25]}...')

        if BatchEncoderDecoder.is_players_batch(batch):
            self.matches_joiner.add_players_batch(batch)
            logging.info(f'Q3 JOINER {self.assigned_shard_key}: Added players batch {body[:25]}...')
        elif BatchEncoderDecoder.is_matches_batch(batch):
            self.matches_joiner.add_matches_batch(batch)
            logging.info(f'Q3 JOINER {self.assigned_shard_key}: Added matches batch {body[:25]}...')
