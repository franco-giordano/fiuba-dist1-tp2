...
class ShardedGrouperController:
    ...
    def run(self):
        logging.info(f'Q3 GROUPER {self.assigned_shard_key}: Waiting for messages. To exit press CTRL+C')
        self.channel.start_consuming()
        self.connection.close()

    def _callback(self, ch, method, properties, body):
        if BatchEncoderDecoder.is_encoded_sentinel(body):
            logging.info(f"Q3 GROUPER {self.assigned_shard_key}: Received sentinel! Flushing and shutting down...")
            self.civ_grouper.received_sentinel()
            # TODO: shutdown my node
            return

        joined_match = BatchEncoderDecoder.decode_bytes(body)
        logging.info(f'Q3 GROUPER {self.assigned_shard_key}: Received joined match {body[:25]}...')
